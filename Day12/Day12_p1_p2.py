...
with open("./Day12/Day12_Input.txt",'r', encoding="utf-8") as file:
    file_parse = file.read().split('\n')
T1 = [(x.split()[0], tuple(map(int, x.split()[1].split(',')))) for x in file_parse]
T2 = [('?'.join([x.split()[0]]*5), tuple(map(int, x.split()[1].split(',')))*5) for x in file_parse]
